document.addEventListener("DOMContentLoaded", function () {
    ...
    loginForm.addEventListener("submit", function (e) {
        ...
        const dummyUser = { name: "John Doe", email: "john@example.com", password: "12345" };

        if (email === dummyUser.email && password === dummyUser.password) {
            localStorage.setItem("loggedInUser", JSON.stringify({ name: dummyUser.name, email: dummyUser.email }));
    ...
    // Handle Signup (Just storing in localStorage for now)
    signupForm.addEventListener("submit", function (e) {
        ...
        localStorage.setItem("loggedInUser", JSON.stringify({ name: fullName, email: email }));
        alert("Signup successful! You are now logged in.");
        accountModal.style.display = "none";
        location.reload(); // Reload to reflect login status
    });
